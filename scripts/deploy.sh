#!/usr/bin/env bash
# Install or remove the vgraph library and functions.
#
...
LIBFILE="$PWD/build/libvgraph.so"
...
if [ "$UNDEPLOY" = no ]; then
    ...
    vsql -X -c "SELECT function_name, is_fenced FROM v_catalog.user_functions
                WHERE schema_name = 'vgraph'
                  AND function_name IN ('gversion','gbuild','gload','ginfo','gnode','gkhop','gkhop_count','gpath','gcomponents','gcomponents_count','gpagerank')
                ORDER BY 1;"
fi
