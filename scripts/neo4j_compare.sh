...
#
#   scripts/neo4j_compare.sh --neo4j_home=DIR [--java_home=DIR] [--rows=N] [--schema=NAME] [--work=DIR]
...
cd "$(dirname "$0")/.."
NEO= JAVA= ROWS=100000000 SCHEMA=VGNEO WORK=/tmp/vgraph_neo4j HEAP=8g PAGECACHE=8g KEEP=no ECHO_ONLY=no
...
CALL vgraph.register_graph('vgneo', '$SCHEMA.contact', ...); CALL vgraph.refresh_graph('vgneo');
...
if [ "$KEEP" = no ]; then
    ...
    sql "CALL vgraph.unregister_graph('vgneo');" > /dev/null
    sql "CALL vgraph.register_graph('vgneo', '$SCHEMA.contact', 'src_id', 'dst_id', NULL, NULL, TRUE, NULL, NULL);
         UPDATE vgraph.manifest SET both_directions = TRUE WHERE graph = 'vgneo'; COMMIT;" > /dev/null
    sql "CALL vgraph.refresh_graph('vgneo');" > /dev/null
    echo "$V_LOAD" > "$WORK/v_load"
fi
V_LOAD=$(cat "$WORK/v_load" 2>/dev/null || echo "?")
V_BUILD=$(sql "SELECT ROUND(build_seconds, 1) FROM vgraph.manifest WHERE graph = 'vgneo';")
V_SIZE=$(sql "SELECT ROUND(SUM(LENGTH(chunk)) / 1e6)::INT FROM vgraph.snapshot WHERE graph = 'vgneo' AND snapshot_id = (SELECT active_snapshot FROM vgraph.manifest WHERE graph = 'vgneo');")
...
cy() {
    "$NEO/bin/cypher-shell" -a bolt://127.0.0.1:7687 --format verbose "$1" 2>&1 | awk '
        /^\|/ { val = $0; gsub(/[| "]/, "", val) }
        /ready to start consuming query after/ { for (i = 1; i <= NF; i++) { if ($i == "after" && $(i + 1) != "another") { a = $(i + 1) } if ($i == "another") { b = $(i + 1) } } }
        END { print val, a + b }'
}
# best of three: prints "<value> <best ms>"
# A question that runs into the transaction timeout (600 s) is not repeated: prints "timeout 600000".
cy3() {
    local best= v= r ms
    for i in 1 2 3; do
        r=$(cy "$1"); v=${r% *}; ms=${r##* }
        case "$v" in ''|*[!0-9]*) echo "timeout 600000"; return 0 ;; esac
...
echo "== questions"
FAILED=0
same() { if [ "$3" != timeout ] && [ "$2" != "$3" ]; then echo "   DIFFERENT RESULT for $1: vgraph $2, Neo4j $3"; FAILED=$((FAILED + 1)); fi; }

for d in 2 3 6 9; do
    r=$(vq "vn_count_$d" "SELECT /*+LABEL(vn_count_$d)*/ SUM(nodes) FROM (SELECT vgraph.gkhop_count(1, $REST USING PARAMETERS graph='vgneo', depth=$d) OVER() FROM one_row) r;")
    ...
    note "people within $d hops of person 1, counted (${r% *})" "$(fmt "${r##* }")" "$(fmt "${n##* }")" "apoc.path.subgraphNodes"
done
for d in 2 3 4 6 9; do
    r=$(vq "vn_count_$d" "SELECT /*+LABEL(vn_count_$d)*/ SUM(nodes) - 1 FROM (SELECT vgraph.gkhop_count(1, $REST USING PARAMETERS graph='vgneo', depth=$d) OVER() FROM one_row) r;")
    n=$(cy3 "MATCH (a:Person {id: 1})-[:KNOWS*1..$d]-(b) WHERE b <> a RETURN count(DISTINCT b)")
    same "plain Cypher $d hops" "${r% *}" "${n% *}"
    nt=$(fmt "${n##* }"); [ "${n% *}" = timeout ] && nt="more than 600 s, stopped"
    note "the same within $d hops, plain Cypher pattern (${r% *})" "$(fmt "${r##* }")" "$nt" "MATCH -[:KNOWS*1..$d]-"
done

# a far target: the smallest id among the people at the largest distance from person 1
TARGET=$(sql "SELECT node FROM (SELECT vgraph.gkhop(1, $REST USING PARAMETERS graph='vgneo', depth=64) OVER() FROM dual) r ORDER BY hops DESC, node LIMIT 1;")
r=$(vq vn_path "SELECT /*+LABEL(vn_path)*/ MAX(hop_no) FROM (SELECT vgraph.gpath(1, $TARGET, NULL::INT, NULL::INT, NULL::BOOLEAN, NULL::FLOAT, NULL::INT, NULL::INT USING PARAMETERS graph='vgneo') OVER() FROM one_row) r;")
...
note "shortest path from person 1 to person $TARGET (${r% *} hops)" "$(fmt "${r##* }")" "$(fmt "${n##* }")" "shortestPath"
...
N_PROJECT=${p% *}
r=$(vq vn_cc "SELECT /*+LABEL(vn_cc)*/ COUNT(*) FROM (SELECT vgraph.gcomponents_count($ARGS USING PARAMETERS graph='vgneo') OVER() FROM one_row) r;")
n=$(cy3 "CALL gds.wcc.stats('g') YIELD componentCount RETURN componentCount")
same "connected components" "${r% *}" "${n% *}"
note "connected components of the whole graph, counted (${r% *})" "$(fmt "${r##* }")" "$(fmt "${n##* }")" "gds.wcc.stats, after the projection"
r=$(vq vn_pr "SELECT /*+LABEL(vn_pr)*/ COUNT(*) FROM (SELECT vgraph.gpagerank($ARGS USING PARAMETERS graph='vgneo', iterations=20, damping=0.85, top=10) OVER() FROM one_row) r;")
n=$(cy3 "CALL gds.pageRank.stats('g', {maxIterations: 20, dampingFactor: 0.85, tolerance: 0}) YIELD ranIterations RETURN ranIterations")
note "PageRank of the whole graph, 20 iterations (vgraph returns the top 10, Neo4j only statistics)" "$(fmt "${r##* }")" "$(fmt "${n##* }")" "gds.pageRank.stats, after the projection"
...
echo "| prepare for graph queries | $V_BUILD s (refresh_graph: build and load the snapshot) | $(fmt "$N_PROJECT") (GDS projection, needed for components and PageRank, repeated after every restart) |"
