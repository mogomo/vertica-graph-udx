// gnode: tells which node holds which rows of the segmented table vgraph.probe.
//   vgraph.gnode(k) OVER(PARTITION NODES) FROM vgraph.probe
// Output (node_name, k): one row per node, with the smallest k stored on it.
// gload uses it to get exactly one probe row, and so one copy of the chunks, per node.
#include "Vertica.h"

using namespace Vertica;

static const char *const FN = "gnode";

class GNode : public TransformFunction
{
...
class GNodeFactory : public TransformFunctionFactory
{
    ...
    { return vt_createFuncObject<GNode>(srvInterface.allocator); }
};

RegisterFactory(GNodeFactory);
