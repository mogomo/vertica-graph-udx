...
only inserted.
    ...
    CREATE TABLE app.contacts (
        ...
        ts   TIMESTAMP NOT NULL DEFAULT SYSDATE()     -- version: the insertion time
    )
...
- The version column must hold the time of the insert (a `DEFAULT SYSDATE()`
  or `NOW()`), not a business date. It may also be an increasing INT.
...
and reading the changes takes 1 to 2 ms on a 100 million row table. Without
...
`margin` is the overlap of the changes, in seconds (default 600). It must be
longer than your longest transaction that writes edges. Rows inside the margin
are applied twice, which is harmless. For an INT version column give the
margin in units of that column.
...
columns to two; they have not been measured again yet.
